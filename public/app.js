...
const mini=document.getElementById('minimap');const mctx=mini?.getContext('2d');function drawMinimap(){if(!mctx)return;const w=mini.width,h=mini.height;mctx.clearRect(0,0,w,h);mctx.fillStyle='rgba(5,12,15,.96)';mctx.fillRect(0,0,w,h);const range=110,scale=Math.min(w,h)/(range*2),map=(x,z)=>[w/2+(x-S.x)*scale,h/2+(z-S.z)*scale];
// The minimap now uses the same generated road/building data as the 3D city.
for(const r of city.roads){const minx=Math.min(r.x1,r.x2),maxx=Math.max(r.x1,r.x2),minz=Math.min(r.z1,r.z2),maxz=Math.max(r.z1,r.z2);if(maxx<S.x-range||minx>S.x+range||maxz<S.z-range||minz>S.z+range)continue;const [x1,y1]=map(r.x1,r.z1),[x2,y2]=map(r.x2,r.z2);mctx.strokeStyle=r.type==='freeway'?'#657477':r.type==='boulevard'?'#829093':'#4c5c60';mctx.lineWidth=Math.max(2,r.width*scale);mctx.beginPath();mctx.moveTo(x1,y1);mctx.lineTo(x2,y2);mctx.stroke()}
